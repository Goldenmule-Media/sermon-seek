...
- Commit as you go. On any task big enough to have parts — a migration plus the code that uses it, a fix plus its test, several independent fixes — commit each part as it passes its checks rather than saving one commit for the end. Each commit should stand on its own: lint and the relevant typecheck/tests pass at that commit, so a bisect never lands on a broken tree. This applies whether or not the request says "commit as you go".
@HOTSEAT.md
