...
export interface EnrichVideoResult {
  topicsInserted: number
  refsInserted: number
  skipped: boolean
}
...
export async function enrichVideo({
  ...
  if (!transcript) return { topicsInserted: 0, refsInserted: 0, skipped: true }
  ...
  await db.transaction().execute(async (trx) => {
    await trx.deleteFrom("video_scripture_refs").where("video_id", "=", videoDbId).execute()
    if (refs.length > 0) {
      await trx
  ...
  if (!force) {
    ...
    if (existing) {
      return { topicsInserted: 0, refsInserted, skipped: true }
    }
  }

  const output = await enricher.enrich(transcript.full_text, title)
  ...
  await db.transaction().execute(async (trx) => {
    ...
    await trx.deleteFrom("video_topics").where("video_id", "=", videoDbId).execute()
  ...
  return { topicsInserted, refsInserted, skipped: false }
}
...
export async function runEnrichBackfill({
  ...
  for (const video of videos) {
    ...
    if (result.skipped && result.refsInserted === 0 && result.topicsInserted === 0) {
      log(`skip ${video.youtube_video_id}: no transcript or already enriched`)
      totals.videosSkipped++
      continue
    }
    totals.topicsInserted += result.topicsInserted
    totals.refsInserted += result.refsInserted
    if (result.skipped) {
