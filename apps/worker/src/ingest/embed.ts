...
export async function embedVideo({
  ...
  if (!transcript) return { chunksInserted: 0, embeddingsInserted: 0, skipped: true }
  ...
  if (segments.length === 0) return { chunksInserted: 0, embeddingsInserted: 0, skipped: true }
  ...
  if (nChunks > 0 && nChunks === nEmbeddings) {
    return { chunksInserted: 0, embeddingsInserted: 0, skipped: true }
  }
  ...
  await db.transaction().execute(async (trx) => {
    ...
    embeddingsInserted += toEmbed.length
  })

  return { chunksInserted, embeddingsInserted, skipped: false }
}
...
export async function runEmbedBackfill({
  ...
  for (const video of videos) {
    ...
    const result = await embedVideo({ db, embedder, churchId, videoDbId: video.id })
    if (result.skipped) {
      log(`skip ${video.youtube_video_id}: no transcript, no segments, or already embedded`)
      totals.videosSkipped++
      continue
    }
    totals.chunksInserted += result.chunksInserted
    totals.embeddingsInserted += result.embeddingsInserted
